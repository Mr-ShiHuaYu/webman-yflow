 ...
 * @since 2023/5/29
 */
class ReductionSignatureAdapter extends AbstractWarmFlowAdapter implements WarmFlowAdapter
{
    /**
     * 判断是否适配指定的流程类型
     *
    ...
    {
         return $warmFlowType == CooperateType::REDUCTION_SIGNATURE->value;
    }
    ...
    public function adapter(array $obj): bool
    {
        ...
        $flowParams->handler($userId)
            ...
            ->reductionHandlers($obj['addHandlers'] ?? [])
            ->message($this->type($obj['operatorType'] ?? 0));

        return $this->taskService->reductionSignature($taskId, $flowParams);
    }
}
